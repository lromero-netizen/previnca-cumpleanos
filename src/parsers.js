...
export function parseTango(buf) {
  ...
  const ci = head.indexOf("CUIL")
  const ni = head.indexOf("NOMBRE")
  const fi = head.indexOf("FECHA")
  const bi = head.indexOf("BANCO")
  const cci = head.indexOf("CENTRO COSTO")
  if (ci < 0 || ni < 0 || fi < 0) throw new Error("Faltan columnas obligatorias")
  const out = []
  for (let i = 1; i < rows.length; i++) {
    const r = rows[i]
    if (!r || !r[ci]) continue
    const cuil = onlyDigits(r[ci])
    if (cuil.length < 11) continue
    out.push({
      cuil,
      nombreRaw: (r[ni] || "").toString().trim(),
      banco: (r[bi] || "").toString().trim(),
      centro: cci >= 0 ? (r[cci] || "").toString().trim() : "",
      fecha: parseDate(r[fi])
    })
  }
...
export function parseArca(text) {
  ...
  for (const ln of lines) {
    if (/^01\s+\d{11}/.test(ln)) salud++
  }
  if (salud > 3) {
    let leg = 0
    for (const ln of lines) {
      const m = ln.match(/^01\s+(\d{11})/)
      if (!m) continue
      byCuil.set(m[1], { name: "", active: true, hasName: false })
      leg++
    }
  ...
  let leg = 0
  for (const ln of lines) {
    const m = ln.match(/^\s*(\d{11})\s+(.+)$/)
    if (!m) continue
    const dates = (ln.match(/\d{2}\/\d{2}\/\d{4}/g) || [])
    byCuil.set(m[1], {
      name: m[2].slice(0, 55).trim(),
