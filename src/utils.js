...
export function parseDate(v) {
  if (v instanceof Date) {
    return { d: v.getDate(), m: v.getMonth(), txt: pad(v.getDate()) + "/" + pad(v.getMonth() + 1) }
  }
...
export function maskCuil(c) {
  const d = onlyDigits(c)
  return d.length >= 11 ? "••-•••••" + d.slice(8) : d
}

export function formatCuil(c) {
  const d = onlyDigits(c)
  return d.length >= 11 ? d.slice(0, 2) + "-" + d.slice(2, 10) + "-" + d.slice(10, 11) : d
}
